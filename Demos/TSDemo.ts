import { I1 } from "./TSDemoInterface1";
import { I2 } from "./TSDemoInterface2";

//Take two objects that have string arrays
//Alternately print out their strings
function AlternatingStrings(ob1: I1, ob2: I2): string {
  //Find the smaller arrays
  let smallArray: number = Math.min(ob1.stuff.length, ob2.stuff.length);
  let fullString: string = '';
  for (let i = 0; i < smallArray; i++) {
    fullString += `${ob1.stuff[i]} ${ob2.stuff[i]} `;
  }
  return fullString;
}

let thing1: I1 = { stuff: ["This", "a", "cool", "demo"], bigNum: 123456789012345612373456n, curDate: new Date() };
let thing2: I2 = { stuff: ["is", "really", "typescript", "."], numStuff: 444, conditional: true };

let resultString = AlternatingStrings(thing1, thing2);

console.log(resultString);
//This is a really cool typescript demo.